...
<head>
	...
	<title>中国竹木工艺网(www.bamboo-crafts.cn)-企业信息</title>
	<meta content="company" name="activemenu" />
	...
	function onBatchDelete(){
		...
		if(confirm("确认删除？")){
			$("#mainForm").attr("action","company!batchDelete.action").submit();
		}
	}
	function onPublish(){
		var  isChecked=false;
		...
		if(confirm("确认发布？")){
			$("#mainForm").attr("action","company!publish.action").submit();
		}
	}
	function onCancel(){
		var  isChecked=false;
		...
		if(confirm("确认取消发布？")){
			$("#mainForm").attr("action","company!cancelPublish.action").submit();
		}
	}
...
<body> 
	<div id="wrapper"> 
		...
		<div id="content"> 
			...
			<h2 class="mb10 box_solid_bottom">企业信息</h2>
				<div id="message" style="line-height: 35px;">
					<s:actionmessage theme="custom" cssClass="tipbox"/>
				</div>
    			<form id="mainForm" action="company.action" method="post">
					...
					<p class="ll mt15">
	    				<button id="btnCreate" type="button" onclick="window.location.href='company!input.action'">新增</button>
    				</p>
					<table class="tablebox">
	    				...
	    				<s:iterator value="page.result" status="st">
						   <tr>
						        ...
						        <td>
						        	<s:if test="ispublic">
						        		<a href="company!input.action?id=${id}&isedit=true">查看</a>
						        	</s:if>
						        	<s:else>
						        		<a href="company!input.action?id=${id}">修改</a>&nbsp;&nbsp;
						        	<a href="javaScript:delRecord('company!delete.action?id=${id}');">删除</a>
						        	</s:else>
						        </td>
						    </tr>
					    </s:iterator>
